...
func (c *Controller) syncHandler(key string) error {
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			utilruntime.HandleError(fmt.Errorf("hello '%s' in work queue no longer exists", key))
			return nil
		}
	...
	// Get the deployment with the name specified in Hello.spec
	...
	// If the Deployment is not controlled by this Hello resource, we should log
...
// to find the Hello resource that 'owns' it. It does this by looking at the
// objects metadata.ownerReferences field for an appropriate OwnerReference.
// It then enqueues that Hello resource to be processed. If the object does not
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		// If this object is not owned by a Hello, we should not do anything more
		// with it.
		if ownerRef.Kind != "Hello" {
			return
		}

		hello, err := c.hellosLister.Hellos(object.GetNamespace()).Get(ownerRef.Name)
		if err != nil {
